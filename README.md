...
> ✅ **v0.11.0** — LSP diagnostics (14 languages), static analysis (9 analyzers), commit audit with CVE-scored severity, optional Antares CVE-localization guard, Anthropic Messages API support, DeepSeek prompt caching telemetry, large-repo hardening (fast-track + `--skip-tier2`), MCP `propagate`, 1257 tests pass.
...
- **Test suite:** ~1257 tests across 30 test files (parallelized with pytest-xdist)
